...
│   • title / subtitle / theme / date  │
│   • partner-logo gallery (image +    │
│     "Select" button per logo)        │
│   • Generate                         │
└──────────────────────────────────────┘
   │ (Generate)
   ▼
Channel:   "🎨 @user generated <Template Name> — file in thread."
   └─ Thread:
        ...
        └─ "Approved by Max"  ← button, click to mark approved
```

The image previews shown in Slack are rendered PNGs served by a small HTTP server inside the bot (`src/lib/static-server.js`) — Slack `image` blocks require a public HTTPS URL.
