...
└──────────────────────────────────────┘
   ...
   ▼
Requester's channel:
   "🎨 @user requested *Template Name* — pending design review by @approver."
   (top-level message, visible to channel — no banner image yet)

Approver's DM with the bot:
   ├─ banner-preview.png    (downscaled thumbnail only)
   └─ [ Approve ]  [ Reject ]

   ── on Approve ─→  Parent message updated to:
                        "✅ @user's *Template* was approved by @approver — file in thread."
                     Thread reply:
                        "🎉 @user your approved banner is ready — *Template*."
                        ⤷ banner.png (full resolution, in thread)

   ── on Reject  ─→  Parent message updated to:
                        "❌ @user's *Template* was rejected by @approver."
                     Ephemeral to requester (only they see):
                        "❌ Your banner request was rejected. Run /banner again."
```

Each preview entry pins its own theme (Light / Dark) and any required variant via `defaults`/`variant` in `src/templates/preview-list.js`, so the user is never asked to pick a theme manually. Approval is gated on `APPROVER_USER_ID`: the approver receives the preview privately in a DM with the bot, and only their click on Approve / Reject is honored. The approved banner is delivered as a thread reply to the original "pending review" message, keeping the channel itself uncluttered.
