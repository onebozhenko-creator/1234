 ...
 *       ↳ submit "banner_submit"  → render banner + thumbnail, send the
 *                                   thumbnail to the approver's DM with
 *                                   Approve/Reject buttons. The requester
 *                                   only sees an ephemeral "submitted for
 *                                   review" notice — nothing is posted in
 *                                   their channel yet.
 *
 *   button "approve_banner"       → only APPROVER_USER_ID can succeed.
 *                                   Uploads the full banner to the
 *                                   original requester's channel (or DM),
 *                                   tagging them.
 *   button "reject_banner"        → only APPROVER_USER_ID can succeed.
 *                                   Notifies the requester ephemerally
 *                                   and discards the rendered file.
 *
...
function registerSlackHandlers(app) {
  ...
  // ── Final submit: private design-review flow ──────────────────────────
  //
  // Goal: the requester sees NOTHING in their channel until the designer
  // approves — at which point the final banner is delivered there. The
  // preview itself never leaves the designer's DM.
  //
  //   1. Ephemeral ack to the requester ("submitted for review").
  //   2. Render full banner + thumbnail.
  //   3. Open a DM with the approver and upload the thumbnail there with
  //      Approve / Reject buttons. The button payload carries the
  //      requester's user/channel so the approve handler can deliver to
  //      the right place.
  //   4. Full file stays on disk until approve (or reject discards it).
  app.view('banner_submit', async ({ ack, view, client, body }) => {
    ...
    try {
      // 1. Tell the requester (only they see this) that review has started.
      if (channelId) {
        await client.chat
          .postEphemeral({
            channel: channelId,
            user: userId,
            text:
              `:hourglass_flowing_sand: Your *${preview.label}* banner has been sent ` +
              `to <@${APPROVER_USER_ID}> for design review. ` +
              `Once approved, the final file will appear in this channel.`,
          })
          .catch(() => {});
      }
      ...
      //    carries everything the approve handler needs to deliver to
      //    the original requester (or to re-render if the file is gone
      //    after a container restart). Slack caps `value` at 2000 chars;
      //    typical payload here is <700.
  ...
  app.action('approve_banner', async ({ ack, body, client, respond }) => {
    ...
    // Upload the full banner to the requester's channel.
    try {
